...
module Make (PC : ParserAndCompiler.S) = struct
  ...
  let certified_smt =
    let doc =
      "Attempt to use the certified SMT backend, falling back on the \
       unverified one"
    in
    Arg.(value & flag & info [ "certified-smt" ] ~doc)
  ...
  let use (term : (unit -> unit) Term.t) : unit Term.t =
    let apply_common
      ...
      Config.certified_smt := certified_smt;
